            ...
            <h3>Promociones</h3>
				...
				<Toolbar class="p-mb-1 bg-white toolbar">
                    ...
                    <template v-slot:right>
                        ...
                        <Button label="Nueva Promocion" icon="pi pi-plus" class="p-button-success p-mr-2 ml-5 orange" @click="nuevo_medio=true" />
                    </template>
				</Toolbar>

                 <DataTable :value="medios_list"
                            stripedRows
                            editMode="cell"
                            class="editable-cells-table mt-5"
                            showGridlines>

                    <Column field="id" header="Numero" footer="Numero"  style="width:50px;padding-left:15px">
                        <template #body="{data}">
                            {{data.numero}}
                        </template>
                    </Column>
                ...
                <Dialog v-model:visible="nuevo_medio" header="Nuevo medio">
                    <div class="p-dialog-content">
                        ...
                        <InputText v-model="medio.descripcion" :class="{'p-invalid': submitted && !medio.descripcion}">
                        </InputText><br>
                        <small class="p-invalid" v-if="submitted && !medio.descripcion">Este campo es  obligatorio.</small>
                    </div>

                    <template #footer>
						<Button label="Cancelar" icon="pi pi-times" class="p-button-text" @click="nuevo_medio = false"/>
						<Button label="Guardar" icon="pi pi-check" class="p-button-text" @click="storeMedio" />
					</template>
...
export default {
    ...
    mounted() {
        this.mediosService.getMedios().then(res => {
    ...
    methods: {
        storeMedio(){
            ...
            if (this.medio.descripcion != undefined) {
                ...
                this.mediosService.storeMedio(this.medio).then(res => {
                    if (res.status) {
                        this.$toast.add({
                            severity:'success',
                            summary:"Medio de pago Creado",
                            detail:"El nuevo medio de pago se registró con éxito!",
                            life:4000
                        });
        ...
        updateCell(id, field, new_value){
            ...
            this.mediosService.updateMedio(data)
                .then(res => {
                    this.$toast.add({
                            severity:'success',
